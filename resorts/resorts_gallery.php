...
if ((isset($_GET['image'])) && ($_GET['image'] != "")) {
  $deleteSQL = sprintf("DELETE FROM resorts_gallery WHERE seno=%s",
  ...
  $deleteGoTo = "resorts_gallery.php?resort=".$_GET['resort'];
...
//$imgName = basename( $_FILES["newsImg"]["name"]);
  ...
  $insertSQL = sprintf("INSERT INTO resorts_gallery (resort_id, image) VALUES (%s, %s)",
  ...
  $insertGoTo = "resorts_gallery.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_resort_gallery = sprintf("SELECT * FROM resorts_gallery WHERE resort_id = %s", GetSQLValueString($colname_resort_gallery, "int"));
...
<body class="body_min">
<h2 class="page_min_head">Gallery - <?php echo $row_resort_getinfo['name']; ?></h2>
...
<div style="width:90px; display:inline-block; vertical-align:top; border:2px solid #CCC; margin:0 5px 5px 0;text-align:center;">
  ...
  <a href="resorts_gallery.php?resort=<?php echo $row_resort_getinfo['seno']; ?>&image=<?php echo $row_resort_gallery['seno']; ?>" onClick="return confirm('Are you sure to delete this?')"><img src="../support/buttons/delete.fw.png"></a></div>
