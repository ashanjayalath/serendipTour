...
if ((isset($_GET['image'])) && ($_GET['image'] != "")) {
  $deleteSQL = sprintf("DELETE FROM resorts_rooms_facilities WHERE seno=%s",
  ...
  $deleteGoTo = "resorts_rooms_facilities.php?resort=".$_GET['resort']."&room=".$_GET['room'];
...
include_once("../support/ak_php_img_lib_1.2.php");
...
$tmpName = pathinfo($_FILES['image']['name'], PATHINFO_FILENAME);
$name = strtoupper($tmpName );	

//$imgName = basename( $_FILES["newsImg"]["name"]);

  $insertSQL = sprintf("INSERT INTO resorts_rooms_facilities (resort_room_id, name, image) VALUES (%s, %s, %s)",
  ...
  $insertGoTo = "resorts_rooms_facilities.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_resort_gallery = sprintf("SELECT * FROM resorts_rooms_facilities WHERE resort_room_id = %s", GetSQLValueString($colname_resort_gallery, "int"));
...
<body class="body_min">
<h2 class="page_min_head">Rooms - Facilities - <?php echo $row_resort_getinfo['name']; ?></h2>
...
<form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1">
  <table width="100%" border="0">
    <tr>
      <td>Image:</td>
      <td colspan="2"><input name="image" type="file" required="required" id="image"></td>
    </tr>
    <tr>
      <td width="100">&nbsp;</td>
...
<div style="width:90px; display:inline-block; vertical-align:top; border:2px solid #CCC; margin:0 5px 5px 0;text-align:center;">
  ...
  <?php echo $row_resort_gallery['name']; ?><br>
  <a href="resorts_rooms_facilities.php?resort=<?php echo $row_resort_getinfo['seno']; ?>&room=<?php echo $_GET['room']; ?>&image=<?php echo $row_resort_gallery['seno']; ?>" onClick="return confirm('Are you sure to delete this?')"><img src="../support/buttons/delete.fw.png"></a></div>
