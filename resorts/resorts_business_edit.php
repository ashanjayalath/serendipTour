...
//$imgName = basename( $_FILES["newsImg"]["name"]);
  ...
  $updateSQL = sprintf("UPDATE resorts_meeting SET resort_id=%s, heading=%s, image=%s, `description`=%s, phone=%s, email=%s, reservation=%s WHERE seno=%s",
  ...
  $updateGoTo = "resorts_business.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_businesss_edit = "-1";
if (isset($_GET['business'])) {
  $colname_businesss_edit = $_GET['business'];
}
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_businesss_edit = sprintf("SELECT * FROM resorts_meeting WHERE seno = %s", GetSQLValueString($colname_businesss_edit, "int"));
$businesss_edit = mysql_query($query_businesss_edit, $kdmcnensasala) or die(mysql_error());
$row_businesss_edit = mysql_fetch_assoc($businesss_edit);
$totalRows_businesss_edit = mysql_num_rows($businesss_edit);
?><!DOCTYPE html>
<html>
<head>
...
<body class="body_min">
<h2 class="page_min_head">Business - <?php echo $row_resort_getinfo['name']; ?></h2>
  ...
  <table width="100%" border="0">
    <tbody>
      <tr>
        ...
        <td width="250"><input name="heading" type="text" required="required" id="heading" value="<?php echo $row_businesss_edit['heading']; ?>"></td>
        <td width="100" align="right">Image:</td>
        <td width="250" align="right"><input name="image" type="file" required="required" id="image">
        <input name="image_old" type="hidden" id="image_old" value="<?php echo $row_businesss_edit['image']; ?>"></td>
      </tr>
      <tr>
        <td>Phone:</td>
        <td><input name="phone" type="text" required="required" id="phone" value="<?php echo $row_businesss_edit['phone']; ?>"></td>
        <td align="right">e-mail:</td>
        <td align="right"><input name="email" type="text" required="required" id="email" value="<?php echo $row_businesss_edit['email']; ?>"></td>
      </tr>
      <tr>
        <td width="100">Description:</td>
        <td width="250">&nbsp;</td>
        <td width="100" align="right">Reservation:</td>
        <td width="250" align="right"><input name="reservation" type="text" required="required" id="reservation" value="<?php echo $row_businesss_edit['reservation']; ?>"></td>
      </tr>
      <tr>
        <td colspan="4"><textarea name="description" required="required" id="description"><?php echo $row_businesss_edit['description']; ?></textarea>
      ...
      <tr>
        ...
        <td width="250">&nbsp;</td>
        <td width="100">&nbsp;</td>
        <td width="250" align="right"><input name="seno" type="hidden" id="seno" value="<?php echo $row_businesss_edit['seno']; ?>">
        <input name="resort_id" type="hidden" id="resort_id" value="<?php echo $row_businesss_edit['resort_id']; ?>">          <input type="submit" name="submit" id="submit" value="Save"></td>
      </tr>
    </tbody>
  </table>
...
mysql_free_result($businesss_edit);
?>
