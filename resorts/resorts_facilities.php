...
if ((isset($_GET['image'])) && ($_GET['image'] != "")) {
  $deleteSQL = sprintf("DELETE FROM resorts_facilities WHERE seno=%s",
  ...
  $deleteGoTo = "resorts_facilities.php?resort=".$_GET['resort'];
...
//$imgName = basename( $_FILES["newsImg"]["name"]);
  ...
  $insertSQL = sprintf("INSERT INTO resorts_facilities (resort_id, name, image) VALUES (%s, %s, %s)",
  ...
  $insertGoTo = "resorts_facilities.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['resort'])) {
  $colname_resort_gallery = $_GET['resort'];
}
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_resort_gallery = sprintf("SELECT * FROM resorts_facilities WHERE resort_id = %s", GetSQLValueString($colname_resort_gallery, "int"));
...
<body class="body_min">
<h2 class="page_min_head">Facilities - <?php echo $row_resort_getinfo['name']; ?></h2>
...
<form action="<?php echo $editFormAction; ?>" method="POST" enctype="multipart/form-data" name="form1">
  <table width="100%" border="0">
    <tr>
      <td>Image:</td>
      <td colspan="2"><input name="image" type="file" required id="image"></td>
    </tr>
    <tr>
      <td width="100">&nbsp;</td>
      <td><label></label></td>
...
<div style="width:90px; display:inline-block; vertical-align:top; border:2px solid #CCC; margin:0 5px 5px 0;text-align:center;">
  ...
  <a href="resorts_facilities.php?resort=<?php echo $row_resort_getinfo['seno']; ?>&image=<?php echo $row_resort_gallery['seno']; ?>" onClick="return confirm('Are you sure to delete this?')"><img src="../support/buttons/delete.fw.png"></a></div>
