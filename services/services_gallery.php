...
if ((isset($_GET['image'])) && ($_GET['image'] != "")) {
  $deleteSQL = sprintf("DELETE FROM services_gallery WHERE seno=%s",
  ...
  $deleteGoTo = "services_gallery.php?service=".$_GET['service'];
...
if (!file_exists('../../images/services/'.$year)) {
    mkdir('../../images/services/'.$year, 0777, true);
}
if (!file_exists('../../images/services/'.$year.'/'.$month)) {
    mkdir('../../images/services/'.$year.'/'.$month, 0777, true);
}
if (!file_exists('../../images/services/'.$year.'/'.$month.'/'.$date)) {
    mkdir('../../images/services/'.$year.'/'.$month.'/'.$date, 0777, true);

}
	
$moveResult = move_uploaded_file($fileTmpLoc, '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.$fileName);
...
$target_file = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.$fileName;
$resized_file = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.'resized_'.$fileName;
$wmax = 720;
$hmax = 405;
...
if (strtolower($fileExt) != "jpg") {
    $target_file = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.'resized_'.$fileName;
    $new_jpg = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.'resized_'.$kaboom[0].'.jpg';
    ak_img_convert_to_jpg($target_file, $new_jpg, $fileExt);
}
// ----------- End Convert to JPG Function -----------
// ---------- Start Image Watermark Function --------
$timeStamp = date('Ymdhis');
$target_file = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.'resized_'.$kaboom[0].'.jpg';
$wtrmrk_file = '../support/wm.png';//"watermark/wm.png";
$new_file = '../../images/services/'.$year.'/'.$month.'/'.$date.'/'.$timeStamp.'.jpg'; //"uploads/protected_".$kaboom[0].".jpg";
ak_img_watermark($target_file, $wtrmrk_file, $new_file);
// ----------- End Image Watermark Function -----------
//Delete Uploaded File
if (file_exists('../../images/services/'.$year.'/'.$month.'/'.$date.'/'.$fileName)){
	unlink('../../images/services/'.$year.'/'.$month.'/'.$date.'/'.$fileName);
}
if (file_exists('../../images/services/'.$year.'/'.$month.'/'.$date.'/resized_'.$fileName)){
	unlink('../../images/services/'.$year.'/'.$month.'/'.$date.'/resized_'.$fileName);;
}
...
$imgName = 'images/services/'.$year.'/'.$month.'/'.$date.'/'.$timeStamp.'.jpg';
	

//$imgName = basename( $_FILES["newsImg"]["name"]);

  $insertSQL = sprintf("INSERT INTO services_gallery (service_id, image) VALUES (%s, %s)",
  ...
  $insertGoTo = "services_gallery.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_service_getinfo = sprintf("SELECT * FROM services WHERE seno = %s", GetSQLValueString($colname_service_getinfo, "int"));
...
mysql_select_db($database_kdmcnensasala, $kdmcnensasala);
$query_service_gallery = sprintf("SELECT * FROM services_gallery WHERE service_id = %s", GetSQLValueString($colname_service_gallery, "int"));
...
<body class="body_min">
<h2 class="page_min_head">Services - <?php echo $row_service_getinfo['heading']; ?></h2>
  ...
  <table width="100%" border="0">
    <tr>
      ...
      <td width="250" align="right"><label>
        <input name="service_id" type="hidden" id="service_id" value="<?php echo $row_service_getinfo['seno']; ?>">
...
<div style="width:90px; display:inline-block; vertical-align:top; border:2px solid #CCC; margin:0 5px 5px 0;text-align:center;">
  ...
  <a href="services_gallery.php?service=<?php echo $row_service_getinfo['seno']; ?>&image=<?php echo $row_service_gallery['seno']; ?>" onClick="return confirm('Are you sure to delete this?')"><img src="../support/buttons/delete.fw.png"></a></div>
